        ...
        <SafeAreaView style={styles.CategoryProducts_section} >
            ...
            <View style={styles.CategoryProductscontainer} >
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (<ProductShowcaseCard id={item.id} title={item.title} des={item.description} price={item.price} rating={item.rating} badge={item.badge} image={item.image} onPress={()=>navigation.navigate("Cakedetails", { product: item })} />)}
                    keyExtractor={item => item.id}
                />
            </View>
        </SafeAreaView>
