...
export default function AdminCategories() {
  ...
  const [form, setForm] = useState({ title: '', imageUrl: '', sortOrder: 0, isActive: true });
  ...
  const save = async () => {
    ...
    setForm({ title: '', imageUrl: '', sortOrder: 0, isActive: true });
    setShowForm(false);
    load();
  };
  const removeOne = async (id) => {
    await fetch(API_URL + `/api/categories/${id}`, { method: 'DELETE', headers: authHeaders(token) });
    load();
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div><h1 style={{ fontWeight: 900, fontSize: '2rem', letterSpacing: '-0.03em', color: '#4a1d7a', marginBottom: 4 }}>Categories</h1><p style={{ color: '#6b556e' }}>Manage category cards.</p></div>
        <button onClick={() => { setForm({ title: '', imageUrl: '', sortOrder: 0, isActive: true }); setShowForm(true); }} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 18px', borderRadius: 10, background: '#4a1d7a', color: '#fff', border: 'none', fontWeight: 700, cursor: 'pointer' }}><Plus size={16} /> Add Category</button>
      </div>
      ...
      {showForm && (
        <div style={{ background: '#fff', borderRadius: 20, padding: 24, boxShadow: '0 4px 24px rgba(74,29,122,0.06)', border: '1px solid #f0ebf5', marginBottom: 24 }}>
          <h3 style={{ marginBottom: 16, color: '#4a1d7a' }}>{form.id ? 'Edit Category' : 'New Category'}</h3>
          ...
          <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 10 }}>
            <input id="cat-active" type="checkbox" checked={form.isActive} onChange={e => setForm({ ...form, isActive: e.target.checked })} />
            <label htmlFor="cat-active" style={{ fontSize: '0.85rem', color: '#4a1d7a' }}>Active</label>
          </div>
